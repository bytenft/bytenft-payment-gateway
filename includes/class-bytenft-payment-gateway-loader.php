...
class BYTENFT_PAYMENT_GATEWAY_Loader
{
	...
	private function __construct()
	{
		...
		// Track thank you page visits for validation guide
	...
	private function bytenft_validate_customer_account( $checkout_data = null ) {
		...
		*/
		if ( is_wp_error( $response ) ) {
			...
			ByteNFT_Payment_Gateway_Logger::warning(
				'Customer account validation WP HTTP error (failing open)',
				[
					'error' => $error_message,
				]
			);
		...
		*/
		...
		/*
		* If service is unavailable or endpoint returned 404/405/500/HTML error page:
		* Fail open gracefully and check local customer data so existing users are recognized.
		*/
		if ( ! is_array( $response_data ) || in_array( $status_code, [ 404, 405, 500, 502, 503, 504 ], true ) ) {

			ByteNFT_Payment_Gateway_Logger::info(
				'Customer account validation service unavailable / bypassed',
				[
			...
			if ( ! empty( $email ) ) {
				...
				if ( $wp_user ) {
					...
					$detected_user_id     = $wp_user->ID;
				}
			}

			if ( ! $is_existing_customer && is_user_logged_in() ) {
				$is_existing_customer = true;
				$detected_user_id     = get_current_user_id();
			}

			if ( ! $is_existing_customer && ! empty( $email ) ) {
			...
			return [
				'valid'                 => true,
				'requires_confirmation' => false,
				'existing_user'         => $is_existing_customer,
	...
	public static function bytenft_get_validation_guide_statuses()
	{
		...
		if ($reset_timestamp > 0) {
			$order_args_failed['date_created'] = '>=' . $reset_timestamp;
		}
		$failed_orders = wc_get_orders($order_args_failed);

		if (!empty($successful_orders) || get_option('bytenft_successful_payment_verified')) {
			$statuses[4] = 'passed';
		} elseif (!empty($failed_orders) || get_option('bytenft_last_payment_status') === 'failed') {
			$statuses[4] = 'failed';
		} else {
			$statuses[4] = 'pending';
		}
		...
		if (get_option('bytenft_thankyou_page_verified') || !empty($successful_orders)) {
			$statuses[5] = 'passed';
		} elseif (!empty($failed_orders) || get_option('bytenft_thankyou_page_status') === 'failed') {
			$statuses[5] = 'failed';
		} else {
			$statuses[5] = 'pending';
		}

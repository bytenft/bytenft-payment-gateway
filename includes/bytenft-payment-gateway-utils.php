...
function bytenft_check_system_requirements()
{
	if (version_compare(phpversion(), BYTENFT_PAYMENT_GATEWAY_MIN_PHP_VER, '<')) {
		return sprintf(
			...
			__('The DFin Sell Payment Gateway plugin requires PHP version %1$s or greater. You are running %2$s.', 'bytenft-payment-gateway'),
	...
	if (!$wc_plugin_version || version_compare($wc_plugin_version, BYTENFT_PAYMENT_GATEWAY_MIN_WC_VER, '<')) {
		return sprintf(
			...
			__('The DFin Sell Payment Gateway plugin requires WooCommerce plugin version %1$s or greater. You are running %2$s.', 'bytenft-payment-gateway'),
			BYTENFT_PAYMENT_GATEWAY_MIN_WC_VER,
